/* ===========================================================
   CYRUSLINKSHUB — SHARED PROFILE CHIP (works on www + dash)
   Cookie-based = profile survives across subdomains
=========================================================== */
(function () {
    var COOKIE = 'clh_cookie';
    var LANDING = 'https://www.cyruslinkshub.com';

    /* inject chip CSS (no HTML edits needed) */
    ...
    function getProfile() { return readCookie(); }
    ...
    /* global logout — clears cookie so BOTH domains log out */
    ...
    window.clhGetProfile = getProfile;

    function render() {
        var p = getProfile();
        var isDash = location.hostname.indexOf('dash.') === 0;

        /* DASH GUARD: no cookie → back to login */
        if (isDash && !p) { location.replace(LANDING + '/login.html'); return; }
